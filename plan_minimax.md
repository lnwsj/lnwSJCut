...
โมดูล `app.py` เป็นจุดเริ่มต้นของโปรแกรมและเป็นส่วนที่รวมทุกองค์ประกอบเข้าด้วยกัน ไฟล์นี้มีขนาดประมาณ 450 บรรทัดและทำหน้าที่สร้างส่วนติดต่อผู้ใช้ด้วย Flet รวมถึงจัดการ event handling และการสื่อสารระหว่างส่วนต่างๆ ของโปรแกรม การออกแบบในปัจจุบันยังไม่มีการแยก controller ออกจาก view อย่างชัดเจน ซึ่งอาจเป็นปัญหาเมื่อโปรแกรมมีขนาดใหญ่ขึ้น
...
โปรแกรมในเวอร์ชันปัจจุบันมีฟีเจอร์หลักที่รองรับการทำงานพื้นฐานของการตัดต่อวิดีโอ ระบบ Media Bin ช่วยให้ผู้ใช้สามารถนำเข้าไฟล์วิดีโอในรูปแบบ mp4, mov และ mkv ได้ โดยจะแสดงรายชื่อไฟล์และระยะเวลาของแต่ละไฟล์ ผู้ใช้สามารถลากไฟล์จาก Media Bin ลงบน Timeline เพื่อเพิ่มคลิปลงในโปรเจกต์

ระบบ Timeline ประกอบด้วยสองแถวหลักคือ V1 (Video Track 1) และ A1 (Audio Track 1) ผู้ใช้สามารถลากคลิปเพื่อจัดลำดับใหม่ แทรกคลิประหว่างคลิปอื่น หรือลบคลิปที่ไม่ต้องการออกไป การแบ่งคลิป (Split) ทำได้โดยเลือกคลิปและใช้ Slider เพื่อกำหนดจุดตัด แล้วกดปุ่ม Split เพื่อแบ่งคลิปออกเป็นสองส่วน

ระบบ Export ใช้ FFmpeg เป็นตัวประมวลผลหลัก โดยใช้ filter complex กับ trim และ atrim เพื่อตัดแต่งแต่ละคลิป และใช้ concat filter เพื่อรวมคลิปเข้าด้วยกัน ผลลัพธ์จะถูก encode ด้วย libx264 สำหรับวิดีโอและ aac สำหรับเสียง และบันทึกเป็นไฟล์ MP4 ในรูปแบบ yuv420p เพื่อความเข้ากันได้กับอุปกรณ์หลากหลาย
...
ข้อจำกัดด้านไฟล์ที่รองรับก็เป็นปัญหาสำคัญ โปรแกรมปัจจุบันรองรับเฉพาะไฟล์ที่มีทั้ง video stream และ audio stream เท่านั้น ไฟล์ audio-only เช่น MP3 หรือ WAV จะไม่สามารถนำเข้าได้ และถ้าคลิปใดไม่มี audio stream การ export ก็จะล้มเหลว ข้อจำกัดนี้ทำให้ผู้ใช้ไม่สามารถทำงานกับโปรเจกต์ที่มีเสียงพากย์ เพลงประกอบ หรือ podcast ได้อย่างสมบูรณ์
...
ในการตัดต่อวิดีโอสมัยใหม่ การทำงานกับไฟล์เสียงแยกต่างหากเป็นสิ่งที่พบได้บ่อยมาก ผู้ตัดต่อวิดีโอมักต้องการเพิ่มเพลงประกอบ หรือ voice-over ที่บันทึกแยกต่างหาก หรือ sound effects ต่างๆ ที่ไม่ได้อยู่ในไฟล์วิดีโอต้นฉบับ การที่ MiniCut ไม่รองรับไฟล์ audio-only ทำให้ผู้ใช้ไม่สามารถสร้างโปรเจกต์ที่มีเสียงประกอบจากแหล่งต่างๆ ได้อย่างสมบูรณ์

ปัญหาเฉพาะที่พบในปัจจุบันคือเมื่อไฟล์วิดีโอบางไฟล์ไม่มี audio stream (เช่น คลิปที่ถ่ายจากกล้องบางรุ่น หรือไฟล์ที่ตัดมาจากแหล่งอื่น) การ export จะล้มเหลวทันที เนื่องจาก FFmpeg concat filter คาดหวัะว่าทุก input จะมีทั้ง video และ audio streams นี่เป็นข้อจำกัดที่ร้ายแรงมากเพราะทำให้โปรแกรมไม่สามารถใช้งานได้กับไฟล์จริงหลายประเภท
...
การรองรับไฟล์ audio-only ต้องมีการปรับแต่งหลายส่วนของระบบ เริ่มจากการขยาย Media Bin ให้สามารถแสดงและจัดการไฟล์เสียงได้ รูปแบบไฟล์ที่ควรรองรับครอบคลุม MP3, WAV, AAC, OGG และ FLAC การนำเข้าไฟล์เสียงควรแสดงข้อมูลเพิ่มเติมเช่น sample rate, bit depth และช่องสัญญาณ (mono/stereo)

ในส่วน Timeline ต้องมีการปรับแต่งเพื่อให้คลิป audio-only แสดงผลได้อย่างถูกต้อง คลิปเสียงควรแสดงเป็นแถบสีที่แตกต่างจากคลิปวิดีโอ (เช่น สีเขียวแทนสีน้ำเงิน) และควรแสดง waveform preview ถ้าเป็นไปได้ Timeline ควรรองรับการลากคลิปเสียงลงบน audio track โดยเฉพาะ หรืออนุญาตให้วางบน video track แล้วระบบจะสร้าง audio track ใหม่อัตโนมัติ

การ export ต้องมีการปรับแต่งอย่างมากเพื่อรองรับไฟล์ที่มีเฉพาะ video, เฉพาะ audio หรือทั้งสองอย่าง วิธีการคือใช้ FFmpeg filter ที่ซับซ้อนขึ้น โดยแยก processing สำหรับ video และ audio ออกจากกัน แล้วค่อย mux รวมกันในขั้นตอนสุดท้าย

สำหรับไฟล์ที่ไม่มี audio stream วิธีแก้คือการสร้าง silent audio segment ด้วย anullsrc filter ของ FFmpeg เพื่อให้ concat filter ทำงานได้ โดยระยะเวลาของ silent segment จะเท่ากับระยะเวลาของ video clip นั้น
...
ขั้นตอนใน Phase 1 ประกอบด้วยการพัฒนา Undo/Redo system ก่อน เพราะเป็นรากฐานสำคัญที่ฟีเจอร์อื่นๆ หลายอย่างอาจต้องใช้ และจากนั้นจึงพัฒนา Audio-only support ซึ่งจะต้องรอ Undo/Redo เสร็จก่อนเพราะมี dependencies ในการปรับ model และ operations

Deliverables ของ Phase 1 คือ Undo/Redo ที่ทำงานได้ครบถ้วน รองรับไฟล์ MP3, WAV และ audio formats อื่นๆ สามารถ export project ที่มี audio-only clips ได้ และไม่มี crash เมื่อพบไฟล์ที่ไม่มี audio stream
...
Flet version ควรอัปเกรดเป็นเวอร์ชันล่าสุดเพื่อรับ bug fixes และ features ใหม่ๆ ควรตรวจสอบ compatibility ก่อน upgrade และควรติดตาม Flet development roadmap
